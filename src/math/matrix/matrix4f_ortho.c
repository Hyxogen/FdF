...
/*   Updated: 2022/01/18 12:47:50 by dmeijer       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
t_matrix4f
	...
	ret.u_data.m_elements[3 + 0 * 4] = -(proj->m_right + proj->m_left)
		/ (proj->m_right - proj->m_left);
	ret.u_data.m_elements[3 + 1 * 4] = -(proj->m_top + proj->m_bottom)
		/ (proj->m_top - proj->m_bottom);
	ret.u_data.m_elements[3 + 2 * 4] = -(proj->m_far + proj->m_near)
